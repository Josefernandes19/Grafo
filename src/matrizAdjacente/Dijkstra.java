...
public class Dijkstra {
    ...
    public Dijkstra(MatrizAdjacente grafo, Vertice[] vertices) {
        this.grafo = grafo;
        this.vertices = vertices;
    }
    
    public Dijkstra(MatrizAdjacente grafo){
        this.grafo = grafo;
    ...
    public void solucao(int atual) throws Exception {
        ...
        for (int count = 0; count < grafo.numeroDeVertices(); count++) {//checa todos os vertices
            ...
            for (int i = 0; i < adj.size(); i++) {//atualizar pai e distancia
                if (vertices[adj.get(i)].distancia > vertices[atual].distancia + grafo.ma[atual][adj.get(i)]) {
                    vertices[adj.get(i)].distancia = vertices[atual].distancia + grafo.ma[atual][adj.get(i)];
                    vertices[adj.get(i)].pai = vertices[atual].index;
                }
            }
            Vertice aux = new Vertice(atual);
